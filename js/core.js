...
export function calculate(state) {
  ...
  const complexity = Math.min(3.8, items.reduce((factor, item) => factor * (item.complexity || 1), 1));
  const breadth = Math.min(1.9, 1 + Math.max(0, items.length - 18) * 0.018);
  const total = Math.round(raw * tier.multiplier * deliveryMode.multiplier * complexity * breadth);
